class tK_RocketMultiFire extends tK_ProjectileFire;
...
{
    Weapon.PlayOwnedSound(Sound'WeaponSounds.RocketLauncher.RocketLauncherLoad', SLOT_None,,,,,false);
}
...
{
    tK_RocketLauncher(Weapon).PlayLoad(false);
}
...
function PlayFiring()
{
    if (Load > 1.0)
        ...
        FireAnim = 'Fire';
    Super.PlayFiring();
    tK_RocketLauncher(Weapon).PlayFiring((Load == MaxLoad));
    Weapon.OutOfAmmo();
}

event ModeDoFire()
{
    if ( tK_RocketLauncher(Weapon).bTightSpread || ((Bot(Instigator.Controller) != None) && (FRand() < 0.65)) )
    {
        Spread = TightSpread;
        SpreadStyle = SS_Ring;
    }
...
function Projectile SpawnProjectile(Vector Start, Rotator Dir)
{
    ...
    p = tK_RocketLauncher(Weapon).SpawnProjectile(Start, Dir);
    if ( P != None )
