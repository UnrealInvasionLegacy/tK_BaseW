class tK_LinkFire extends tK_WeaponFire;
...
simulated function bool myHasAmmo( tK_LinkGun LinkGun )
{
...
simulated function float AdjustLinkDamage( tK_LinkGun LinkGun, Actor Other, float Damage )
{
...
function PlayFiring()
{
    if (tK_LinkGun(Weapon).Links <= 0 && Weapon.AmmoAmount(ThisModeNum) >= AmmoPerFire)
...
function SetLinkTo(Pawn Other)
{
    ...
    {
        RemoveLink(1 + tK_LinkGun(Weapon).Links, Instigator);
        tK_LinkGun(Weapon).Linking = false;
    }
    ...
    if (LockedPawn != None)
    {
        if (!AddLink(1 + tK_LinkGun(Weapon).Links, Instigator))
        {
            bFeedbackDeath = true;
        }
        tK_LinkGun(Weapon).Linking = true;
...
function bool AddLink(int Size, Pawn Starter)
{
    ...
    if (LockedPawn != None && !bFeedbackDeath)
    {
        ...
        {
            Inv = LockedPawn.FindInventoryType(class'tK_LinkGun');
            if (Inv != None)
            {
                if (LinkFire(tK_LinkGun(Inv).GetFireMode(1)).AddLink(Size, Starter))
                    tK_LinkGun(Inv).Links += Size;
                else
                    return false;
            }
        }
...
function RemoveLink(int Size, Pawn Starter)
{
    ...
    if (LockedPawn != None && !bFeedbackDeath)
    {
        if (LockedPawn != Starter)
        {
            ...
            if (Inv != None)
            {
                LinkFire(tK_LinkGun(Inv).GetFireMode(1)).RemoveLink(Size, Starter);
                tK_LinkGun(Inv).Links -= Size;
            }
        }
    }
...
function bool IsLinkable(Actor Other)
{
    ...
    {
        P = Pawn(Other);
        if ( P.Weapon == None || !P.Weapon.IsA('tK_LinkGun') )
        {
            if ( Vehicle(P) != None )
        ...
        while ( LF != None && LF.LockedPawn != None && LF.LockedPawn != P && sanity < 32 )
        {
            ...
            LG = tK_LinkGun(LF.LockedPawn.Weapon);
            if ( LG == None )
                break;
            LF = LinkFire(LG.GetFireMode(1));
            sanity++;
