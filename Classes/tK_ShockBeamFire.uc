class tK_ShockBeamFire extends tK_InstantFire;
...
function DoFireEffect()
{
    ...
    if ( PlayerController(Instigator.Controller) != None )
    {
       ...
       Weapon.GetViewAxes(X,Y,Z);
        StartTrace = StartTrace + X*class'tK_ShockProjFire'.Default.ProjSpawnOffset.X;
        if ( !Weapon.WeaponCentered() )
            StartTrace = StartTrace + Weapon.Hand * Y*class'tK_ShockProjFire'.Default.ProjSpawnOffset.Y + Z*class'tK_ShockProjFire'.Default.ProjSpawnOffset.Z;
    }
...
{
    if ( (tK_ShockRifle(Weapon) != None) && (tK_ShockRifle(Weapon).ComboTarget != None) )
        return Rotator(tK_ShockRifle(Weapon).ComboTarget.Location - Start);
