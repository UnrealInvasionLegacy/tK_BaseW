...
// Link Gun
//=============================================================================
class tK_LinkGun extends tK_Weapon;
...
{
    if ( FireMode[1] != None )
        tK_LinkFire(FireMode[1]).UpdateLinkColor( Color );
...
simulated function Destroyed()
{
    ...
    {
        if ( tK_LinkFire(FireMode[1]) != None )
            tK_LinkFire(FireMode[1]).SetLinkTo(None);
    }
    Super.Destroyed();
}
...
function bool FocusOnLeader(bool bLeaderFiring)
{
    ...
    if ( (Vehicle(LeaderPawn) != None)
        || ((tK_LinkGun(LeaderPawn.Weapon) != None) && ((vector(B.Squad.SquadLeader.Rotation) dot Normal(Instigator.Location - LeaderPawn.Location)) < 0.9)) )
    {
        StartTrace = Instigator.Location + Instigator.EyePosition();
        if ( VSize(LeaderPawn.Location - StartTrace) < tK_LinkFire(FireMode[1]).TraceRange )
        {
...
function float GetAIRating()
{
    ...
    if ( (PlayerController(B.Squad.SquadLeader) != None)
        ...
        && (tK_LinkGun(B.Squad.SquadLeader.Pawn.Weapon) != None) )
        return 1.2;

    V = B.Squad.GetLinkVehicle(B);
    if ( (V != None)
        && (VSize(Instigator.Location - V.Location) < 1.5 * tK_LinkFire(FireMode[1]).TraceRange)
        && (V.Health < V.HealthMax) && (V.LinkHealMult > 0) )
        return 1.2;

    if ( Vehicle(B.RouteGoal) != None && B.Enemy == None
         && VSize(Instigator.Location - B.RouteGoal.Location) < 1.5 * tK_LinkFire(FireMode[1]).TraceRange
    ...
    if ( O != None && B.Enemy == None && O.TeamLink(B.GetTeamNum()) && O.Health < O.DamageCapacity
         && VSize(Instigator.Location - O.Location) < 1.1 * tK_LinkFire(FireMode[1]).TraceRange && B.LineOfSightTo(O) )
        return 1.2;
...
function byte BestMode()
{
    ...
    if ( V == B.Target )
        return 1;
    if ( (V != None) && (VSize(Instigator.Location - V.Location) < tK_LinkFire(FireMode[1]).TraceRange)
        && (V.Health < V.HealthMax) && (V.LinkHealMult > 0) && B.LineOfSightTo(V) )
        return 1;
    if ( B.Enemy == None )
        return 0;
    EnemyDist = VSize(B.Enemy.Location - Instigator.Location);
    if ( EnemyDist > tK_LinkFire(FireMode[1]).TraceRange )
        return 0;
    return 1;
}
...
function bool LinkedTo(tK_LinkGun L)
{
    local Pawn Other;
    local tK_LinkGun OtherWeapon, Head;
    local int sanity;

    Head = self;
    while (Head != None && Head.Linking && sanity < 20)
    {
            Other = tK_LinkFire(Head.FireMode[1]).LockedPawn;
            if (Other == None)
                return false;
            else
            {
                OtherWeapon = tK_LinkGun(Other.Weapon);
                if (OtherWeapon == None)
                    return false;
                else
                    Head = OtherWeapon;
...
function bool ConsumeAmmo(int Mode, float load, optional bool bAmountNeededIsMax)
{
    ...
    if (Linking && tK_LinkFire(FireMode[1]).LockedPawn != None && Vehicle(tK_LinkFire(FireMode[1]).LockedPawn) == None)
        return true;
    ...
    if (Instigator != None && Instigator.PlayerReplicationInfo != None && Instigator.PlayerReplicationInfo.Team != None)
    {
        for (C = Level.ControllerList; C != None; C = C.NextController)
            if (C.Pawn != None && tK_LinkGun(C.Pawn.Weapon) != None && tK_LinkGun(C.Pawn.Weapon).LinkedTo(self))
                tK_LinkGun(C.Pawn.Weapon).LinkedConsumeAmmo(Mode, load, bAmountNeededIsMax);
    }
...
simulated event WeaponTick(float dt)
{
    ...
    local tK_LinkFire LF;

    PC = PlayerController(Instigator.Controller);
    LF = tK_LinkFire(FireMode[1]);
...
defaultproperties
{
     FireModeClass(0)=Class'tK_LinkAltFire'
     FireModeClass(1)=Class'tK_LinkFire'
     PutDownAnim="PutDown"
