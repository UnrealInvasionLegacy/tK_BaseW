class tK_PainterFire extends tK_WeaponFire;
...
state Paint
{
    function Rotator AdjustAim(Vector Start, float InAimError)
    {
        ...
        if ( Bot(Instigator.Controller) != None )
        {
            ...
            if ( bAlreadyMarked )
                ...
                Instigator.Controller.FocalPoint = tK_Painter(Instigator.Weapon).MarkLocation;
    ...
    function ModeTick(float dt)
    {
                ...
                if (Other.bWorldGeometry)
                {
                    if (VSize(HitLocation - MarkLocation) < 50.0)
                    {
                        ...
                        if (Level.TimeSeconds - MarkTime > 0.3)
                        {
                            ...
                            if ( IonCannon == None )
                                IonCannon = tK_Painter(Weapon).CheckMark(HitLocation, bEngageCannon);
                            ...
                            if ( (IonCannon != None) &&  IonCannon.CheckMark(Instigator,HitLocation,bEngageCannon))
                            {
                                if ( IonCannon.IsFiring() )
                                {
                                    ...
                                    tK_Painter(Weapon).ReallyConsumeAmmo(ThisModeNum, 1);
        ...
        tK_Painter(Weapon).EndEffect = EndEffect;
