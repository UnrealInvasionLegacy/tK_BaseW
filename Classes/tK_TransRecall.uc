class tK_TransRecall extends tK_WeaponFire;
...
simulated function bool AllowFire()
{
    ...
    success = ( tK_TransLauncher(Weapon).AmmoChargeF >= 1.0 );

    if (!success && (Weapon.Role == ROLE_Authority) && (tK_TransLauncher(Weapon).TransBeacon != None) )
    {
...
function Translocate()
{
    ...
    if ( (Instigator == None) || (tK_Translauncher(Weapon) == None) )
        return;
    TransBeacon = tK_TransLauncher(Weapon).TransBeacon;
    ...
    {
        tK_TransLauncher(Weapon).ReduceAmmo();
